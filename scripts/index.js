...
const mensajeError = document.querySelector("#error-container");
const h1 = document.querySelector("h1");
const datos = baseDeDatos.usuarios;
...
function mantenerSesion(){
  if (login == null){
    ...
    h1.innerText = `Bienvenide al sitio ${login.name}`;
    cerrarSesion();
  };
};
...
form.addEventListener( "submit", (e) => {
  ...
  e.preventDefault();
  // Validar
  if (validarEmail(email.value) && validarPassword(email.value, password.value)){
    // Simular inicio de sesión
    iniciando();
    setTimeout(() => {
      form.classList.add("hidden");
      h1.innerText = `Bienvenide al sitio ${usuario.name}`;
      cerrarSesion();
...
function error () {
  ...
  mensajeError.innerHTML = `<small>Algunos de los datos ingresados son incorrectos</small>`;
};

/* Cerrar sesión */
function cerrarSesion(){
  btn.innerHTML = `<button class="logout-btn">Cerrar sesion</button>`
